...
class EncoderBlock(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv1 = nn.Conv2d(
            in_ch, out_ch, 3, stride=1, padding=1, bias=False)
...
class AttentionBlock(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.psi = nn.Sequential(
            nn.Conv2d(out_ch, 1, kernel_size=1, bias=False),
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
    ...
    def forward(self, up_in, feats):
        soft_weights = self.psi(
            self.relu(self.conv_in(up_in) + self.conv_feat(feats)))

        return feats*soft_weights
...
class DecoderBlock(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.att = AttentionBlock(in_ch, out_ch)
        self.conv1 = nn.Conv2d(2 * out_ch, out_ch, 3, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(out_ch)
    ...
    def forward(self, x, features):
        ...
        features = self.att(x, features)
        x = self.relu(self.bn1(self.conv1(torch.cat([x, features], dim=1))))
